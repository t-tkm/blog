...
(※)死亡前死因分析とは、プロジェクトや計画において、事前に失敗要因や問題点を特定するための分析手法の一つです。
具体的には、あたかもプロジェクトが失敗した場合の原因を探るかのように、事前にシナリオを考え、問題の発生要因を洗い出すことで、
失敗リスクを事前に軽減することを目的とします。
...
# 追伸
最近、近所の本屋を探索していると、リチャード・セイラー氏、キャス・サンスティーン氏による「[NUDGE 実践 行動経済学 完全版](https://www.amazon.co.jp/dp/4296000985)」
という書籍が目に留まりました。早速購入し、パラパラと眺めてみたところ、ウェルビーングの達成に「選択アーキテクト(※)」が
関与していく世界観が述べられていました。この時、chatGPTのような生成系AIが、強力なサポーターになるのではと感じました。
